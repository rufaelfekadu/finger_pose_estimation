...
from hpe.util.data import read_emg_v1, read_leap, build_emg_columns, build_leap_columns, strided_array
...
def _prepare_data( data_path, label_path, index=0, lock=None, event=None):
    ...
    data = data.groupby('gesture').filter(lambda x: x.isnull().sum()[label_columns[0]] < 300 ).apply(lambda x: x.fillna(x.mean()))
    ...
    #  normalise again
    data[data_columns] = MinMaxScaler().fit_transform(data[data_columns])
