...
class Transformer(nn.Module):
    ...
    def make_mask(src_input, trg_input):
        e_mask = torch.ones(src_input.shape, dtype=torch.bool).unsqueeze(1).to(device=device)  # (B,1, L)
        d_mask = torch.ones(trg_input.shape, dtype=torch.bool).unsqueeze(1).to(device=device)  # (B, 1, L)
...
if __name__ == "__main__":
    # Test
    # src_input = torch.randint(0, 100, (batch_size, seq_len)).to(device=device)
    # trg_input = torch.randint(0, 100, (batch_size, seq_len)).to(device=device)
    src_input = torch.randn(batch_size, seq_len, 100).to(device=device)
    trg_input = torch.randn(batch_size, seq_len, 100).to(device=device)
    e_mask = torch.ones(batch_size, seq_len).to(device=device)
    d_mask = torch.ones(batch_size, seq_len).to(device=device)

    transformer = Transformer(100, 100)
    output = transformer(src_input, trg_input)
    print(output.shape) # (B, L, trg_vocab_size)
    print(output[0, 0, :]) # (trg_vocab_size)
